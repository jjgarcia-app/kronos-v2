...
la base real (sin mocks), las cuatro cosas que de verdad importan de esta
...
|---|---|---|
| **1. Bloque core** | que `SessionStart` inyecte items del proyecto, que ningún item venga cortado (`recortado por presupuesto`) y cuántos items entraron | el bloque llegó a tener 6-7 items de ATISA sobre 12 en un proyecto que no era ATISA, y a cortar una línea al medio por presupuesto (ronda 4) |
| **2. Recall** | que un prompt conversacional traiga memoria y que un prompt trivial no gaste nada, con la latencia real | los prompts conversacionales no inyectaban **nada** (el FTS usaba AND implícito y solo matcheaban queries de 2 palabras), y el camino vectorial podía comerse 1,5 s con Ollama cargado (ronda 3) |
