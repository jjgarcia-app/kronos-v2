...
## Captura de memoria: cuatro niveles, ninguno reemplaza al agente decidiendo guardar
...
3. **Captura pasiva por compactación** (`RunPreCompactCapture`, `internal/hooks/pre_compact_capture.go`) — justo antes de que Claude Code compacte el contexto, le pregunta al LLM de generación configurado si el tramo reciente del transcript documenta algo guardable, y si sí lo guarda como `type: passive`. Dispara vía POST al daemon (fire-and-forget, 500ms de timeout); si el daemon no responde a tiempo (reiniciando, ocupado), el propio hook corre la misma lógica localmente y sincrónico antes de salir (acotado a 15s) — sin esto, cualquier compactación que coincidiera con un reinicio del daemon perdía la captura en silencio para siempre, sin aviso.
4. **Digest corriente por sesión** (`MaybeUpdateDigest`, `internal/hooks/digest.go`) — no espera a la compactación: cada `UserPromptSubmit`, si pasaron ≥`digest.interval_minutes` (default 20) desde la última actualización (o nunca hubo una), le pide al LLM de generación configurado que extienda un resumen corriendo de la sesión con el tramo reciente del transcript. Se guarda como una sola observación `type: session` con `topic_key: session/<id>` (upsert — nunca crea una fila nueva por actualización, sube `revision_count`), así `mem_search`/`mem_context` encuentran el hilo de en qué se viene trabajando sin depender de que el agente llame `mem_save`/`mem_session_summary` a mano. En el daemon corre en una goroutine desacoplada de la respuesta HTTP con presupuesto propio `digest.timeout_ms` (default 60s, más generoso que `llm.timeout_ms` porque nada del lado del usuario espera este resultado); en el camino de fallback local corre sincrónico, con el timeout general (`llm.timeout_ms`), acotado igual que el punto 3. `IsDigestDue` (chequeo barato, sin LLM) evita pagar el costo de construir el cliente en cada prompt cuando la gran mayoría de las veces todavía no corresponde actualizar. Si `Digest.LLMEnrichment` está apagado o el LLM no responde, igual se guarda una versión determinística (prompts/archivos/comandos extraídos del transcript sin pasar por ningún LLM) — nunca se pierde el digest por completo.

   **Hechos tipados promovidos desde el digest** (`digest.promote_facts`, default true) — la misma llamada al LLM que genera la prosa del digest también le pide hasta `digest.max_facts` (default 3) hechos standalone (bugfix/decision/config/discovery/pattern/preference), que se guardan como observaciones PROPIAS con su tipo, no enterradas en el digest `type: session` (que la inyección automática limita a uno, ver `core.max_session_items`/`recall.max_session_items` más abajo). No agrega una llamada nueva al LLM ni al contador de uso.

   **Reintento de enriquecimiento pendiente** (`internal/llm.DigestPending`) — el enriquecimiento por LLM puede terminar de tres formas: (a) éxito con hechos (se guardan); (b) éxito en la prosa pero sin una respuesta explícita sobre hechos (ni una lista, vacía o no, ni el literal `FACTS: ninguno`) — sin distinguir esto, el aprendizaje se perdía en silencio (medido en producción: digest con prosa, cero hechos, sin nada pendiente); (c) la llamada falló o se pasó del timeout. Los casos (b) y (c) dejan la sesión anotada en un archivo del data dir (mismo mecanismo de Usage/Breaker: lectura tolerante a corrupción, escritura atómica), con un `Kind` que distingue **enriquecimiento completo** (reintentar todo, caso c) de **solo hechos** (caso b: la prosa ya está guardada, el próximo tick manda un pedido acotado — solo el excerpt del transcript, sin volver a pedir la prosa — que pide `FACTS: ninguno` como respuesta explícita si no hay nada). Tope de 3 intentos y expiración a las 12h en ambos casos, para no insistir contra un LLM roto ni reintentar sesiones abandonadas hace días. `kronos doctor` desglosa la línea del digest como `pendientes: N enriquecimiento, M hechos` (o `pendientes: al día` sin ninguno).

### Proveedor del LLM de generación (digest + captura pasiva + judge)

Los tres consumidores anteriores (captura pasiva, digest y `mem_judge`/`AutoJudge`) comparten la misma elección de proveedor vía `llm.provider` (`internal/llm.NewGenerationClientFromConfig` para los dos primeros, que necesitan el tipo concreto `*llm.Client`; `internal/llm.NewFromConfig` para el judge, que solo necesita la interfaz `Judger`):

- `"ollama"` (default) — HTTP local contra `/api/generate`, sin configuración adicional.
- `"claude-cli"` — invoca `claude -p --model <llm.model>` (default `haiku`) como subproceso, con el prompt por stdin. Pensado para máquinas donde Ollama no responde bajo carga (medido en producción: una generación mínima de 60 tokens no terminó en 175s con 6 CPUs y load ~9) pero donde ya hay una suscripción de Claude Code autenticada — evita pagar una API aparte.

**Aislamiento del subprocess (`CLAUDE_CONFIG_DIR`)**: cada invocación de `claude -p` corre con la variable de entorno `CLAUDE_CONFIG_DIR` apuntando a `<data_dir>/claude-cli/`, un directorio de configuración separado del `~/.claude` real, con su propio `settings.json` (`{"hooks":{}}`) y symlinks a las credenciales (`~/.claude/.credentials.json`, `~/.claude.json`). Esto es imprescindible, no cosmético: sin aislar la config, el subproceso hereda los hooks de kronos instalados en el `~/.claude` real (`UserPromptSubmit`, `PreCompact`, etc.) y terminaría disparándolos desde dentro de la propia llamada de kronos al LLM — recursión, y cada capa gastando otra llamada de generación. Verificado en vivo que `--settings '{"hooks":{}}'` NO alcanza (Claude Code igual mezcla la config del `~/.claude` real y los hooks corren), y que con `CLAUDE_CONFIG_DIR` aislado no se crea ninguna sesión nueva de kronos por la llamada del subproceso.

**Guardián de carga** (`internal/llm.LoadGuardStatus`, config `llm.max_load_per_cpu`, default 1.0) — antes de cualquier llamada de generación (Ollama o claude-cli), se compara `load1` de `/proc/loadavg` contra `runtime.NumCPU()`; si `load1/NumCPU` supera el umbral, la llamada se saltea sin intentarla (logueado en debug, sin spam). Motivado por un bug real medido en producción: con la máquina saturada, cada intento fallido dejaba un proceso de generación local girando a >200% CPU durante minutos, empeorando la carga y haciendo fallar el intento siguiente — un círculo vicioso que el guardián corta desde el lado de kronos en vez de esperar a que cada llamada agote su timeout. `0` desactiva el guardián. **Solo protege al modelo local**: `claude-cli` genera en la nube (lo único local es un proceso esperando red), así que saltearlo por carga alta no libera ningún CPU real y en cambio deja la máquina sin captura automática — el guardián no le aplica. Tampoco aplica a los embeddings del recall, que ya degradan a búsqueda FTS por presupuesto de tiempo en vez de por carga.

**Contador de uso y diagnóstico de fallas** (`internal/llm.Usage`/`internal/llm.LastFailure`, visibles en `kronos doctor`) — cada llamada de generación (digest, captura pasiva, judge) queda contada por proveedor y resultado (`ok`, `error`, `skipped_load`, `skipped_breaker`, `skipped_no_creds`) en un archivo de buckets por hora del data dir, con retención de 7 días — sin esto no había forma de ver cuánto se consume de la suscripción de Claude Code (`claude -p`) sin leer el archivo a mano. Las fallas de `claude-cli` además se clasifican (no logueado, CLI/modelo incompatibles, timeout, binario no encontrado, permiso denegado, sin credenciales, desconocido) y la última clasificación persiste aparte del cortacircuitos, para que `kronos doctor` siga mostrando "último fallo: hace X" aunque el proveedor ya se haya recuperado.

**Logger del daemon** — `kronos serve --daemon-mode` redirige el logger por default de `log/slog` a `<data_dir>/daemon.log` en vez de dejarlo en el stderr original del proceso lanzado detached: sin esto, los `slog.Warn`/`slog.Debug` de todo lo anterior (fallas de LLM, guardián de carga, pendientes de digest) se perdían en el stderr de un proceso sin terminal asociada.
...
| `llm` | Clientes de LLM: Ollama (local, siempre disponible sin config), claude-cli (subproceso `claude -p`, config dir aislado), OpenAI, Anthropic |
